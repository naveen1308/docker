...
LABEL io.k8s.description="Platform for building Mule ESB EE Applications" \
      io.k8s.display-name="Mule ESB 3.8.0 Enterprise" \
...
RUN yum update -y \
    ...
    && yum clean all -y \
    && cd /opt \
    && curl -o mule.tar.gz https://s3-us-west-2.amazonaws.com/mule-esb-runtime/mule-ee-distribution-standalone-3.8.0.tar.gz \
